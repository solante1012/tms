...
public class UserController extends BaseController {
	...
	private RespBody createUser(String role, String baseURL, UserForm userForm) {
		...
		try {
			mailSender
					.sendHtmlByQueue(String.format("DCST-用户创建_%s", DateUtil.format(new Date(), DateUtil.FORMAT7)),
	...
	public RespBody batchMail(@RequestParam("baseURL") String baseURL, @RequestParam("users") String users,
		...
		try {
			mailSender.sendHtmlByQueue(String.format("DCST-系统消息_%s", DateUtil.format(new Date(), DateUtil.FORMAT7)),
